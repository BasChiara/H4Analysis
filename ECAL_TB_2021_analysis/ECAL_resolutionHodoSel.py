...
LP_dict_energy_hodoY = {'25':-7.0, '50':-7.0, '75' :-7.0, '100':-7.0,'125':-6.5, '150' : -6.5, '175':-7.0, '200':-7.0}
LP_crystal = 'C2'
C2matrix_3x3 = 'B1,B2,B3,C3,C2,C1,D1,D3,D2'.split(',') 


# HIGH PURITY energy scan #
dict_C3_energy   = {'50' : [14907,14914,14932], '100' : [14918], '150' : [14943,14934], '200': [14951], '250': [14820,14821]}
dict_C3_energy   = {'50' : [14932], '100' : [14918], '150' : [14943,14934], '200': [14951], '250': [14820,14821]}
HP_trees_path = '/eos/cms/store/group/dpg_ecal/comm_ecal/upgrade/testbeam/ECALTB_H4_Oct2021/HighPurity/ntuples_fitVFEs_fixes/'
HP_dict_energy_Nbins   = {'50' : 800, '100': 800, '150': 800, '200' :500, '250' : 500}
HP_dict_energy_hodoX = {'50' : -5.5, '100': 3.5, '150':  3.5, '200': 3.5, '250': 1.5}
HP_dict_energy_hodoY = {'50' : -4.5, '100':-5.5, '150': -4.5, '200':-5.5, '250':-6.5}
if(args.G1): 
    dict_C3_energy['100'] = [15038]#, '150': [14943,14934],'200': [14951], '250': [14820,14821]} # G1-forzato
    HP_dict_energy_Nbins['200'] = 500
    HP_dict_energy_Nbins['100'] = 400
HP_crystal = 'C3'
...
for energy in energies :
    ...
    myCB.set_position(HodoX[energy], HodoY[energy], 2.0 if (args.mode == 'HP') else 1.5) 
    ...
    if(args.mode == 'LP'):
        ...
        if (E>75): 
            myCB.xaxis_scale = 0.075
            myCB.n_min = 1; myCB.n_max = 10; myCB.n_initial = 5 
        if (E>100): 
            myCB.xaxis_scale = 0.070
        ...
        if(E>150): 
            ...
            myCB.n2_min = 0.01; myCB.n2_max = 15; myCB.n2_initial = 5 
            myCB.a_min = 0.05; myCB.a2_max = 10.; myCB.a2_initial = 1.5
    ## HIGH PURITY ###
    if(args.mode == 'HP'):
        myCB.gain = 1
        if(args.G1 and E < 200):continue
        if(args.G1): myCB.gain = 10
    ...
    canvas_num+=1
    print("  canvas slot : ",canvas_num)
        
    if(canvas_num % 4 == 0 or energy == energies[-1]):
       n_canvas += 1
       c.Draw()
...
print(' [OUT] fit parameters saved in results_hodo/%sresults_%s.json'%(crystal,outstr))
